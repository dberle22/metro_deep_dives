-- foundation.tract_features.sql
-- Purpose: Produce tract-level feature spine for Retail Opportunity Finder V1.
-- Params:
--   {{target_year}} : e.g., 2024
-- Notes:
-- - Managed foundation-layer copy of the legacy tract feature query.
-- - Refactored to build a tract feature spine once per target year from the
--   upstream tract universe currently present in silver.xwalk_tract_county.
-- - Market-specific consumers should filter by cbsa_code from the published table.
-- - Target upstream end state is a national ref/geo backbone:
--   silver.xwalk_tract_county should be national, and
--   metro_deep_dive.geo.tracts_all_us should become the canonical tract geometry
--   source before this asset is repointed away from the compatibility table.

CREATE OR REPLACE TABLE rof_features.tract_features AS 

WITH
/* 1) National tract universe with CBSA mapping where available */
tract_universe AS (
  SELECT
    t.tract_geoid,
...
pop_pctl AS (
  SELECT
    tract_geoid,
    PERCENT_RANK() OVER (PARTITION BY cbsa_code, year ORDER BY pop_growth_3yr) AS pop_growth_pctl
  FROM base
...
income_pctl AS (
  SELECT
    tract_geoid,
    PERCENT_RANK() OVER (PARTITION BY cbsa_code, year ORDER BY median_hh_income) AS income_pctl
  FROM base
...
rent_pctl AS (
  SELECT
    tract_geoid,
    PERCENT_RANK() OVER (PARTITION BY cbsa_code, year ORDER BY median_gross_rent) AS rent_pctl
  FROM base
...
value_pctl AS (
  SELECT
    tract_geoid,
    PERCENT_RANK() OVER (PARTITION BY cbsa_code, year ORDER BY median_home_value) AS value_pctl
  FROM base
...
density_pctl AS (
  SELECT
    tract_geoid,
    PERCENT_RANK() OVER (PARTITION BY cbsa_code, year ORDER BY pop_density) AS density_pctl
  FROM base
