...
- Record chart-build decisions in the chart folder (for example `visual_library/charts/<chart_type>/<chart_type>_decisions.md`) instead of the global build plan.
...
- Owner: `Agent`
- Action:
  - Immediately create the standard chart folder structure for the selected chart type under `visual_library/charts/<chart_type>/`.
  - Include `sample_output/` and `sample_sql/` subfolders plus expected starter files when missing.
...
- Output: Approved business questions and sample SQL in `sample_sql/`.
...
- Action:
  ...
  - Update the chart question coverage file and confirm the central analytics manifest still maps all question themes.
...
- `sample_sql/build_<chart_type>_sample.sql`
- `sample_output/test_<chart_type>_business_questions.md`
- `sample_output/test_<chart_type>_render.R` or central registry-driven coverage in `visual_library/run_visual_library_tests.R`
