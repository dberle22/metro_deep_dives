-- rof_features.tract_model_audit.sql
-- Purpose: Build tract model scoring diagnostics from rof_features.tract_features.

CREATE OR REPLACE TABLE rof_features.tract_model_audit AS
...
imputed AS (
  SELECT
    ...
    price_proxy_pctl,
    density_pctl,
    CASE WHEN pop_growth_3yr IS NULL THEN 1 ELSE 0 END AS imputed_pop_growth_flag,
    CASE WHEN permits_per_1k_3yr IS NULL THEN 1 ELSE 0 END AS imputed_permits_flag,
    CASE WHEN density IS NULL THEN 1 ELSE 0 END AS imputed_density_flag,
    CASE WHEN price_proxy IS NULL THEN 1 ELSE 0 END AS imputed_price_flag,
    CASE WHEN commute_intensity IS NULL THEN 1 ELSE 0 END AS imputed_commute_flag,
    CASE WHEN median_household_income IS NULL THEN 1 ELSE 0 END AS imputed_income_flag,
    COALESCE(
      pop_growth_3yr,
...
scored AS (
  SELECT
    ...
    0.35 * z_pop_growth AS contrib_balanced_pop_growth,
    0.25 * z_permits AS contrib_balanced_permits,
    0.15 * z_density_inv AS contrib_balanced_density_inv,
    0.10 * z_price_inv AS contrib_balanced_price_inv,
    0.10 * z_income AS contrib_balanced_income,
    0.05 * z_commute AS contrib_balanced_commute,
    0.45 * z_pop_growth AS contrib_growth_pop_growth,
    0.25 * z_permits AS contrib_growth_permits,
    0.10 * z_income AS contrib_growth_income,
    0.10 * z_density_inv AS contrib_growth_density_inv,
    0.05 * z_price_inv AS contrib_growth_price_inv,
    0.05 * z_commute AS contrib_growth_commute,
    0.25 * z_pop_growth AS contrib_value_pop_growth,
    0.20 * z_permits AS contrib_value_permits,
    0.10 * z_density_inv AS contrib_value_density_inv,
    0.30 * z_price_inv AS contrib_value_price_inv,
    0.10 * z_income AS contrib_value_income,
    0.05 * z_commute AS contrib_value_commute,
    0.30 * z_commute AS contrib_corridor_commute,
    0.25 * z_pop_growth AS contrib_corridor_pop_growth,
    0.15 * z_permits AS contrib_corridor_permits,
    0.10 * z_density_inv AS contrib_corridor_density_inv,
    0.10 * z_income AS contrib_corridor_income,
    0.10 * z_price_inv AS contrib_corridor_price_inv
  FROM gated
),
...
ranked AS (
  SELECT
    ...
    density_gate_flag,
    eligible_flag,
    contrib_balanced_pop_growth,
    contrib_balanced_permits,
    contrib_balanced_density_inv,
    contrib_balanced_price_inv,
    contrib_balanced_income,
    contrib_balanced_commute,
    contrib_growth_pop_growth,
    contrib_growth_permits,
    contrib_growth_income,
    contrib_growth_density_inv,
    contrib_growth_price_inv,
    contrib_growth_commute,
    contrib_value_pop_growth,
    contrib_value_permits,
    contrib_value_density_inv,
    contrib_value_price_inv,
    contrib_value_income,
    contrib_value_commute,
    contrib_corridor_commute,
    contrib_corridor_pop_growth,
    contrib_corridor_permits,
    contrib_corridor_density_inv,
    contrib_corridor_income,
    contrib_corridor_price_inv,
    contrib_balanced_pop_growth +
      contrib_balanced_permits +
      contrib_balanced_density_inv +
      contrib_balanced_price_inv +
      contrib_balanced_income +
      contrib_balanced_commute AS score_balanced,
    contrib_growth_pop_growth +
      contrib_growth_permits +
      contrib_growth_income +
      contrib_growth_density_inv +
      contrib_growth_price_inv +
      contrib_growth_commute AS score_growth,
    contrib_value_pop_growth +
      contrib_value_permits +
      contrib_value_density_inv +
      contrib_value_price_inv +
      contrib_value_income +
      contrib_value_commute AS score_value,
    contrib_corridor_commute +
      contrib_corridor_pop_growth +
      contrib_corridor_permits +
      contrib_corridor_density_inv +
      contrib_corridor_income +
      contrib_corridor_price_inv AS score_corridor
  FROM scored
)
