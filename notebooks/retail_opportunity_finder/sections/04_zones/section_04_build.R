...
scored_required <- c(
  ...
  "pop_total", "pop_growth_5yr", "pop_density", "units_per_1k_3yr", "price_proxy_pctl"
)
tract_sf_required <- c("tract_geoid", "eligible_v1")
component_required <- c(
  "tract_geoid", "eligible_v1", "is_scored",
  "pop_growth_5yr", "pop_density", "units_per_1k_3yr", "price_proxy_pctl"
)
...
eligible_from_scored <- scored_tracts %>%
  filter(eligible_v1 == 1) %>%
  distinct(tract_geoid)

eligible_from_components <- tract_component_scores %>%
  filter(eligible_v1 == 1) %>%
  distinct(tract_geoid)

eligible_from_geom <- tract_sf %>%
  sf::st_drop_geometry() %>%
  filter(eligible_v1 == 1) %>%
  distinct(tract_geoid)

missing_scored_in_geom <- setdiff(eligible_from_scored$tract_geoid, eligible_from_geom$tract_geoid)
missing_geom_in_scored <- setdiff(eligible_from_geom$tract_geoid, eligible_from_scored$tract_geoid)
missing_component_in_geom <- setdiff(eligible_from_components$tract_geoid, eligible_from_geom$tract_geoid)
...
readiness_report <- list(
  ...
  counts = list(
    ...
    eligible_from_scored = nrow(eligible_from_scored),
    eligible_from_geom = nrow(eligible_from_geom),
    eligible_from_components = nrow(eligible_from_components),
...
# Step 2: Define zone candidate universe (all eligible tracts)
...
if (nrow(zone_candidate_tracts) == 0) {
  stop("No eligible tracts available for zone candidate universe.", call. = FALSE)
}
...
message(glue::glue("Section 04 build step 2 complete: {nrow(zone_candidate_tracts)} eligible tracts selected as zone candidates."))
...
zone_summary <- zone_metric_base %>%
  ...
  arrange(zone_order) %>%
  mutate(
    pop_growth_5yr_wtd_fmt = scales::percent(pop_growth_5yr_wtd, accuracy = 0.1),
  ...
  select(
    ...
    pop_growth_5yr_wtd, pop_growth_5yr_wtd_fmt,
