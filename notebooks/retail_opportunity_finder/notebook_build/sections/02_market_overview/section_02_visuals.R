...
# Notebook-build output rule:
# Write visual artifacts into the notebook_build tree directly so it is obvious
# where this migrated script saves files, without depending on shared legacy
# output helpers.
NB_SECTION_OUTPUT_DIR <- file.path(
...
# Small local helper for this script only.
# Example:
#   nb_output_path("section_02_visual_objects")
#   nb_output_path("section_02_pop_trend_plot", ext = "png")
nb_output_path <- function(name, ext = "rds", subdir = NULL) {
  dir_path <- NB_SECTION_OUTPUT_DIR

  if (!is.null(subdir) && nzchar(subdir)) {
    dir_path <- file.path(dir_path, subdir)
  }

  file_name <- if (!is.null(ext) && nzchar(ext)) {
    paste0(name, ".", ext)
  } else {
    name
  }
...
ggplot2::ggsave(
  filename = nb_output_path("section_02_pop_trend_plot", ext = "png"),
  plot = pop_trend_plot,
  width = 9,
  height = 5,
  dpi = 150
...
ggplot2::ggsave(
  filename = nb_output_path("section_02_market_context_map", ext = "png"),
...
ggplot2::ggsave(
  filename = nb_output_path("section_02_market_context_map_style_a", ext = "png"),
...
ggplot2::ggsave(
  filename = nb_output_path("section_02_market_context_map_style_b", ext = "png"),
...
ggplot2::ggsave(
  filename = nb_output_path("section_02_distribution_plot", ext = "png"),
