...
# Step D1: Input readiness and canonicalization
zone_path <- "notebooks/retail_opportunity_finder/sections/04_zones/outputs/section_04_zones.rds"
zone_summary_path <- "notebooks/retail_opportunity_finder/sections/04_zones/outputs/section_04_zone_summary.rds"
...
zones <- readRDS(zone_path)
zone_summary <- readRDS(zone_summary_path)
cluster_zones <- readRDS(cluster_zone_path)
cluster_zone_summary <- readRDS(cluster_zone_summary_path)

zone_schema_check <- validate_columns(
  zones,
  c("zone_id", "zone_label", "geometry"),
  "section_04_zones"
)
...
zone_summary_schema_check <- validate_columns(
  zone_summary,
  c("zone_id", "zone_label", "mean_tract_score"),
...
zone_geom_check <- validate_sf(zones, "section_04_zones", storage_crs_epsg)
...
cluster_zones_canonical <- cluster_zones %>%
  transmute(
    zone_system = "cluster",
...
zones_canonical <- dplyr::bind_rows(zones_canonical, cluster_zones_canonical) %>%
  sf::st_make_valid() %>%
...
readiness_report <- list(
  ...
  schema_checks = list(
    zone_schema_check = zone_schema_check,
    cluster_zone_schema_check = cluster_zone_schema_check,
    zone_summary_schema_check = zone_summary_schema_check,
  ...
  geometry_checks = list(
    zone_geom_check = zone_geom_check,
  ...
  pass = isTRUE(zone_schema_check$pass) &&
    isTRUE(cluster_zone_schema_check$pass) &&
    isTRUE(zone_summary_schema_check$pass) &&
    isTRUE(cluster_zone_summary_schema_check$pass) &&
    isTRUE(zone_geom_check$pass) &&
...
message(glue::glue(
  "Section 05 D1 complete: {nrow(zones_canonical)} canonical zones and {nrow(parcels_canonical)} canonical parcels."
))
...
# Use parcel interior points for deterministic tract assignment and to avoid
# duplicate assignment where parcel polygons cross tract boundaries.
# All spatial operations in Section 05 use the configured analysis CRS.
...
# Step D3: Zone overlays and parcel shortlist candidates
...
zone_components_check <- validate_columns(
  zone_components,
  c("tract_geoid", "zone_component_id"),
...
if (!isTRUE(zone_components_check$pass) || !isTRUE(cluster_assignments_check$pass)) {
  stop("Section 05 D3 zone mapping inputs failed schema checks.", call. = FALSE)
}

contiguity_tract_zone_map <- zone_components %>%
  left_join(
    zone_summary %>%
      select(zone_component_id, zone_id, zone_label, mean_tract_score),
...
cluster_tract_zone_map <- cluster_assignments %>%
  transmute(
    tract_geoid = as.character(tract_geoid),
    zone_system = "cluster",
    zone_id = as.character(cluster_id),
    zone_label = as.character(cluster_label),
    mean_tract_score = as.numeric(NA_real_)
  ) %>%
  left_join(
    cluster_zone_summary %>%
      transmute(
...
tract_zone_map <- bind_rows(contiguity_tract_zone_map, cluster_tract_zone_map)
...
zone_overlay_contiguity <- tract_zone_map %>%
  filter(zone_system == "contiguity") %>%
...
zones_contiguity_proj <- normalize_for_spatial_ops(
  zones_canonical %>% filter(zone_system == "contiguity"),
  "zones_contiguity",
  analysis_crs_epsg
)
zones_cluster_proj <- normalize_for_spatial_ops(
  zones_canonical %>% filter(zone_system == "cluster"),
  "zones_cluster",
  analysis_crs_epsg
)

parcel_zone_contiguity <- sf::st_join(
  retail_points %>% select(parcel_uid),
  zones_contiguity_proj %>% select(zone_id, zone_label),
  join = sf::st_within,
  left = FALSE
) %>%
  sf::st_drop_geometry() %>%
...
parcel_zone_assignment <- bind_rows(parcel_zone_contiguity, parcel_zone_cluster) %>%
...
parcel_shortlist_candidates <- parcel_zone_assignment %>%
  left_join(parcel_tract_map, by = "parcel_uid") %>%
  left_join(
    retail_classified_parcels %>%
      sf::st_drop_geometry() %>%
      select(parcel_uid, parcel_id, county, county_name, land_use_code, retail_subtype, parcel_area_sqmi, assessed_value, last_sale_date, last_sale_price),
    by = "parcel_uid"
  ) %>%
  left_join(
...
parcel_shortlist_scored <- parcel_shortlist_candidates %>%
  ...
  mutate(
    ...
    pctl_assessed_value = safe_percent_rank(assessed_value),
    inv_pctl_assessed_value = 1 - pctl_assessed_value,
...
parcel_shortlist_contiguity <- parcel_shortlist_scored %>%
  filter(zone_system == "contiguity") %>%
...
shortlist_report <- list(
  run_metadata = run_metadata(),
  checks = list(
    zone_components_check = zone_components_check,
    cluster_assignments_check = cluster_assignments_check,
    tract_zone_map_key_check = tract_zone_map_key_check
  ),
  counts = list(
    overlay_contiguity_zones = nrow(zone_overlay_contiguity),
    overlay_cluster_zones = nrow(zone_overlay_cluster),
    parcel_zone_assignments = nrow(parcel_zone_assignment),
    shortlist_contiguity_rows = nrow(parcel_shortlist_contiguity),
    shortlist_cluster_rows = nrow(parcel_shortlist_cluster)
  ),
  pass = isTRUE(zone_components_check$pass) &&
    isTRUE(cluster_assignments_check$pass) &&
...
message(glue::glue(
  "Section 05 D3 complete: {nrow(parcel_shortlist_contiguity)} contiguity and {nrow(parcel_shortlist_cluster)} cluster parcel candidates."
))
