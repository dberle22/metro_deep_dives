...
This checklist is the operating tracker for the `data_platform` table review. A table is considered reviewed when we have:
- profiled the current materialized DuckDB table
- documented what is currently in it
- created or confirmed the table-owned `.sql` or `.R` asset we want to manage going forward

Legend:
- `Profiled`: whether we have inspected the live DuckDB table shape, row counts, and coverage
- `Managed asset`: whether the table has a named table-owned `.sql` or `.R` file in the layer `tables/` directory
- `Reviewed`: whether the table has completed the current review pass

| Layer | Physical product | Profiled | Managed asset | Reviewed | Notes |
| --- | --- | --- | --- | --- | --- |
| 2.0 | `ref.market_profiles` | `No` | `No` | `No` | |
| 2.0 | `ref.market_cbsa_membership` | `No` | `No` | `No` | |
| 2.0 | `ref.market_county_membership` | `No` | `No` | `No` | |
| 2.0 | `ref.county_dim` | `No` | `No` | `No` | |
| 2.0 | `ref.tract_dim` | `No` | `No` | `No` | |
| 2.0 | `ref.land_use_mapping` | `No` | `No` | `No` | |
| 2.0 | `qa.ref_validation_results` | `No` | `No` | `No` | |
| 2.0 | `qa.ref_geography_coverage` | `No` | `No` | `No` | |
| 2.0 | `qa.ref_unmapped_land_use_codes` | `No` | `No` | `No` | |
| 2.1 | `foundation.cbsa_features` | `Yes` | `Yes` | `Yes` | Rebuilt on `2026-04-06`; table-owned SQL lives at `tables/foundation.cbsa_features.sql`; live table is the managed national foundation metro feature service. |
| 2.1 | `foundation.tract_features` | `Yes` | `Yes` | `Yes` | Rebuilt on `2026-04-06`; table-owned SQL lives at `tables/foundation.tract_features.sql`; live table reflects the current upstream tract backbone (`10573` rows, `106` non-null CBSAs, `553` null `cbsa_code` rows). |
| 2.1 | `foundation.market_tract_geometry` | `Yes` | `Yes` | `Yes` | Rebuilt on `2026-04-06`; table-owned SQL lives at `tables/foundation.market_tract_geometry.sql`; live table publishes CBSA-keyed tract geometry from the current upstream tract backbone (`10020` rows, `106` CBSAs). |
| 2.1 | `foundation.market_county_geometry` | `Yes` | `Yes` | `Yes` | Rebuilt on `2026-04-06`; table-owned SQL lives at `tables/foundation.market_county_geometry.sql`; live table publishes national CBSA-partitioned county geometry (`1915` rows, `935` CBSAs). |
| 2.1 | `foundation.market_cbsa_geometry` | `Yes` | `Yes` | `Yes` | Rebuilt on `2026-04-06`; table-owned SQL lives at `tables/foundation.market_cbsa_geometry.sql`; live table publishes national CBSA geometry (`935` rows). |
| 2.1 | `foundation.context_cbsa_boundary` | `Yes` | `Yes` | `Yes` | Transitional cached R object profiled on `2026-04-06`; lightweight reader lives at `tables/foundation.context_cbsa_boundary.R`; no longer republished into DuckDB by the foundation layer. |
| 2.1 | `foundation.context_county_boundary` | `Yes` | `Yes` | `Yes` | Transitional cached R object profiled on `2026-04-06`; lightweight reader lives at `tables/foundation.context_county_boundary.R`; no longer republished into DuckDB by the foundation layer. |
| 2.1 | `foundation.context_places` | `Yes` | `Yes` | `Yes` | Transitional cached R object profiled on `2026-04-06`; lightweight reader lives at `tables/foundation.context_places.R`; no longer republished into DuckDB by the foundation layer. |
| 2.1 | `foundation.context_major_roads` | `Yes` | `Yes` | `Yes` | Transitional cached R object profiled on `2026-04-06`; lightweight reader lives at `tables/foundation.context_major_roads.R`; no longer republished into DuckDB by the foundation layer. |
| 2.1 | `foundation.context_water` | `Yes` | `Yes` | `Yes` | Transitional cached R object profiled on `2026-04-06`; lightweight reader lives at `tables/foundation.context_water.R`; no longer republished into DuckDB by the foundation layer. |
| 2.1 | `qa.foundation_validation_results` | `Yes` | `Yes` | `Yes` | Rebuilt on `2026-04-06`; QA table now validates state coverage, duplicate keys, and tract-to-geometry coverage gaps in the managed foundation outputs. |
| 2.1 | `qa.foundation_geography_coverage` | `No` | `Yes` | `Yes` | Added on `2026-04-06`; state-level tract backbone coverage summary for `silver.xwalk_tract_county`, tract features, and tract geometry sources. |
| 2.1 | `qa.foundation_null_rates` | `Yes` | `Yes` | `Yes` | Rebuilt on `2026-04-06`; QA table now publishes null-rate summaries for the live managed feature tables. |
| 2.2 | `scoring.tract_scores` | `Yes` | `Yes` | `Yes` | Rebuilt on `2026-04-06`; table-owned R builder now lives at `tables/scoring.tract_scores.R`; live table publishes a southeast multi-market scoring slice (`11172` rows across `117` markets / CBSAs), and the updated Layer 03 runner no longer overwrites it. |
| 2.2 | `scoring.cluster_seed_tracts` | `Yes` | `Yes` | `Yes` | Rebuilt on `2026-04-06`; table-owned R builder now lives at `tables/scoring.cluster_seed_tracts.R`; live table publishes the current `25%` seed rule across the same southeast multi-market slice (`2836` rows across `117` markets / CBSAs), and Layer 03 now consumes it read-only. |
| 2.3 | `zones.zone_input_candidates` | `Yes` | `Yes` | `Yes` | Rebuilt and re-profiled on `2026-04-06`; table-owned R asset now lives at `tables/zones.zone_input_candidates.R`; live table publishes `2629` geometry-bearing candidate tracts across `115` zone-ready markets / CBSAs in `FL`, `GA`, `NC`, and `SC`, with `2` skipped markets still blocked by tract-geometry readiness gaps. |
| 2.3 | `zones.contiguity_zone_components` | `Yes` | `Yes` | `Yes` | Rebuilt and re-profiled on `2026-04-06`; table-owned R asset now lives at `tables/zones.contiguity_zone_components.R`; live table assigns all `2629` candidate tracts across the `115` published markets into contiguity components with zero duplicate tract keys. |
| 2.3 | `zones.contiguity_zone_summary` | `Yes` | `Yes` | `Yes` | Rebuilt and re-profiled on `2026-04-06`; table-owned R asset now lives at `tables/zones.contiguity_zone_summary.R`; live table publishes `749` contiguity zones across the current multi-market southeast slice. |
| 2.3 | `zones.contiguity_zone_geometries` | `Yes` | `Yes` | `Yes` | Rebuilt and re-profiled on `2026-04-06`; table-owned R asset now lives at `tables/zones.contiguity_zone_geometries.R`; live table publishes `749` dissolved contiguity geometries across the current multi-market southeast slice. |
| 2.3 | `zones.cluster_assignments` | `Yes` | `Yes` | `Yes` | Rebuilt and re-profiled on `2026-04-06`; table-owned R asset now lives at `tables/zones.cluster_assignments.R`; live table assigns all `2629` candidate tracts across `115` markets into cluster zones with zero duplicate tract keys. |
| 2.3 | `zones.cluster_zone_summary` | `Yes` | `Yes` | `Yes` | Rebuilt and re-profiled on `2026-04-06`; table-owned R asset now lives at `tables/zones.cluster_zone_summary.R`; live table publishes `447` cluster zones across the current multi-market southeast slice. |
| 2.3 | `zones.cluster_zone_geometries` | `Yes` | `Yes` | `Yes` | Rebuilt and re-profiled on `2026-04-06`; table-owned R asset now lives at `tables/zones.cluster_zone_geometries.R`; live table publishes `447` dissolved cluster geometries across the current multi-market southeast slice. |
| 2.3 | `qa.zone_build_validation_results` | `Yes` | `Yes` | `Yes` | Rebuilt and re-profiled on `2026-04-06`; Layer 03 now publishes `18` structured QA checks covering market coverage, duplicate keys, row-parity, and summary/geometry consistency. |
| 2.3 | `qa.zone_build_skipped_markets` | `Yes` | `Yes` | `Yes` | Rebuilt and re-profiled on `2026-04-06`; Layer 03 now publishes the `2` skipped markets with structured readiness-gap counts instead of hiding them in runner logs. |
| 2.4 | `parcel.parcels_canonical` | `No` | `No` | `No` | |
| 2.4 | `parcel.parcel_join_qa` | `No` | `No` | `No` | |
| 2.4 | `parcel.parcel_lineage` | `No` | `No` | `No` | |
| 2.4 | `parcel.retail_parcels` | `No` | `No` | `No` | |
| 2.4 | `qa.parcel_validation_results` | `No` | `No` | `No` | |
| 2.4 | `qa.parcel_unmapped_use_codes` | `No` | `No` | `No` | |
| 2.5 | `serving.retail_parcel_tract_assignment` | `No` | `No` | `No` | |
| 2.5 | `serving.retail_intensity_by_tract` | `No` | `No` | `No` | |
| 2.5 | `serving.parcel_zone_overlay` | `No` | `No` | `No` | |
| 2.5 | `serving.parcel_shortlist` | `No` | `No` | `No` | |
| 2.5 | `serving.parcel_shortlist_summary` | `No` | `No` | `No` | |
| 2.5 | `qa.market_serving_validation_results` | `No` | `No` | `No` | |
...
| 2.0 QA | Reference geography coverage | `qa.ref_geography_coverage` | one row per `state_fips`, `state_abbr` | State-level summary of the tract dimension backbone. This is the fast way to see whether `ref.tract_dim` is still southeastern-only or has actually reached national coverage. | Counts tract and county rows by state from `ref.tract_dim`. | Geography QA, national expansion tracking | Implemented |
...
| Layer | Logical product | Physical product | Grain | Description | Main logic / role | Main consumers | Status |
| --- | --- | --- | --- | --- | --- | --- | --- |
| 2.1 Foundation | Metro feature service | `foundation.cbsa_features` | one row per `cbsa_code`, `year` | Metro-level feature table for the market overview story. This is where the notebook gets Jacksonville-at-a-glance KPIs like population, growth, rent, home value, and commute metrics. | Builds and publishes the national DuckDB table `foundation.cbsa_features` from the table-owned asset `data_platform/layers/01_foundation_features/tables/foundation.cbsa_features.sql`. Downstream consumers should read the DuckDB table, not the build file. | Section 02, future metro snapshots | Implemented |
| 2.1 Foundation | Tract feature service | `foundation.tract_features` | one row per `cbsa_code`, `tract_geoid`, `year` | Tract-level analytical feature table that feeds tract eligibility and scoring. This is the central “model input” table containing growth, density, price proxy, commute, income, gate flags, and other tract KPIs. | Builds and publishes a DuckDB tract feature spine from the table-owned asset `data_platform/layers/01_foundation_features/tables/foundation.tract_features.sql`, partitioning percentile logic by `cbsa_code` so downstream consumers can filter the published table for the target market. Intended end state is national once the upstream tract backbone is national. | Scoring layer, Section 03 compatibility path | Implemented |
| 2.1 Foundation | Market tract geometry | `foundation.market_tract_geometry` | one row per `cbsa_code`, `tract_geoid` | Generalized tract geometry service in DuckDB-friendly `geom_wkt` form. This is the geometry handoff used for maps, tract-level joins, and zone construction without forcing every section to re-query raw geometry sources. | Builds and publishes a CBSA-partitioned tract geometry table from `data_platform/layers/01_foundation_features/tables/foundation.market_tract_geometry.sql`; downstream consumers should filter by `cbsa_code`. Canonical all-tract geometry should live upstream, not here. | Scoring, zones, Section 02/03/04 compatibility | Implemented |
| 2.1 Foundation | Market county geometry | `foundation.market_county_geometry` | one row per `cbsa_code`, `county_geoid` | Generalized county geometry service for county outlines and context overlays in the notebook. | Builds and publishes a national CBSA-partitioned county geometry table from `data_platform/layers/01_foundation_features/tables/foundation.market_county_geometry.sql`; downstream consumers should filter by `cbsa_code`. | Section 02, Section 05 map context | Implemented |
| 2.1 Foundation | Market CBSA geometry | `foundation.market_cbsa_geometry` | one row per `cbsa_code` | Generalized CBSA boundary geometry service for outline display and high-level map framing. | Builds and publishes a national CBSA geometry table from `data_platform/layers/01_foundation_features/tables/foundation.market_cbsa_geometry.sql`; downstream consumers should filter by `cbsa_code`. | Section 02 maps, future serving products | Implemented |
| 2.1 Foundation | Optional context layers | `foundation.context_cbsa_boundary`, `foundation.context_county_boundary`, `foundation.context_places`, `foundation.context_major_roads`, `foundation.context_water` | geometry row grain varies by source | These are notebook cartography helpers, not model inputs. They preserve the roads, places, and water layers needed to make the final visuals readable without re-owning context ingestion inside each section. | Documents and reads existing Section 02 context artifacts through lightweight table-owned `.R` readers; these assets remain transitional and are not republished into DuckDB by the foundation layer. | Section 02, Section 05 visuals | Transitional |
| 2.1 QA | Foundation validation | `qa.foundation_validation_results` | one row per QA check | Audit table for required columns, key uniqueness, publication coverage, and geometry compatibility in the foundation layer. | Emits structured QA results for foundation products and now flags whether the upstream tract universe has reached national state coverage. | Ops review, build validation | Implemented |
| 2.1 QA | Foundation geography coverage | `qa.foundation_geography_coverage` | one row per `state_fips`, `state_abbr` | State-level backbone QA table for tract coverage. It compares the upstream tract universe, the tract feature publication, the tract geometry source, and the CBSA-keyed geometry serving table so we can see exactly where national expansion is blocked. | Summarizes counts and gaps by state across `silver.xwalk_tract_county`, `foundation.tract_features`, `metro_deep_dive.geo.tracts_supported_states`, and `foundation.market_tract_geometry`. | Geography QA, build review, migration planning | Implemented |
...
| 2.4 Parcel | Parcel join QA | `parcel.parcel_join_qa` | one row per `market_key`, `county_geoid` | County-grain QA bridge for parcel geometry readiness retained for downstream compatibility. This still tells downstream consumers which county geometry artifacts are available and how cleanly geometry joined during the county parcel workflow. | Compatibility projection derived from `parcel.parcel_lineage`. | Section 05 geometry lookup, parcel QA review | Implemented |
| 2.4 Parcel | Parcel lineage | `parcel.parcel_lineage` | one row per `market_key`, `county_geoid` | Primary county-grain operational lineage table for parcel publication. It records where the county parcel load came from, which load log entries contributed to the current market parcel set, county geometry QA status, and how many parcels were published per county. | Joins market counties, county QA artifacts, county load log, and canonical parcel counts into a publication lineage view. | Ops review, debugging, audit, compatibility QA projection | Implemented |
| 2.4 Parcel | Retail parcels | `parcel.retail_parcels` | one row per `parcel_uid` | Retail-only subset of canonical parcels retained for downstream compatibility. | Filters `parcel.parcels_canonical` to `retail_flag = TRUE`. | Serving layer, Section 05 | Implemented |
